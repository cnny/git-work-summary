...
class GitAnalyzer {
     ...
     * 获取所有本地分支
     */
    async getAllLocalBranches(repoPath) {
        try {
            if (!this.git) {
                this.git = (0, simple_git_1.default)(repoPath);
            }
     ...
     * 获取最近的提交信息
     */
    async getRecentCommits(repoPath, maxCommits, onlyMyCommits = false, scanAllBranches = false) {
        try {
            this.git = (0, simple_git_1.default)(repoPath);
            // 检查是否为 Git 仓库
            ...
            if (scanAllBranches) {
                ...
                // 转换为数组并按时间排序
                allCommits = Array.from(commitMap.values()).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
     ...
     * 根据日期范围获取提交记录
     */
    async getCommitsByDateRange(repoPath, startDate, endDate, onlyMyCommits = false, scanAllBranches = false) {
        try {
            this.git = (0, simple_git_1.default)(repoPath);
            // 检查是否为 Git 仓库
            const isRepo = await this.git.checkIsRepo();
     ...
     * 获取工作区状态
     */
    async getWorkspaceStatus(repoPath) {
        try {
            this.git = (0, simple_git_1.default)(repoPath);
     ...
     * 设置最后处理的提交
     */
    setLastProcessedCommit(commitHash) {
        this.lastProcessedCommit = commitHash;
    }
    /**
     * 获取未提交的变更内容
     */
    async getUncommittedChanges(repoPath) {
        try {
            if (!this.git) {
                this.git = (0, simple_git_1.default)(repoPath);
            }
     ...
     * 获取特定提交的变更内容
     */
    async getCommitChanges(repoPath, commitHash) {
        try {
            if (!this.git) {
                this.git = (0, simple_git_1.default)(repoPath);
            }
     ...
     */
    isMeaninglessCommitMessage(message) {
