...
exports.WorkSummaryStorage = void 0;
const fs = __importStar(require("fs/promises"));
const path = __importStar(require("path"));
class WorkSummaryStorage {
    constructor(context) {
     ...
     * 清理过期的工作总结
     */
    async cleanupOldSummaries(keepDays = 90) {
        try {
            const summaries = await this.loadAllSummaries();
     ...
     * 导入工作总结数据
     */
    async importSummaries(filePath, merge = true) {
        try {
    ...
    async loadAllSummaries() {
        try {
            ...
            return summaries.map((summary) => ({
                ...
                commits: Array.isArray(summary.commits) ? summary.commits.map((commit) => ({
                    ...commit,
    ...
    async getSummariesByDateRange(startDate, endDate) {
        try {
            ...
            return summaries.filter(summary => summary.timestamp >= startTime && summary.timestamp <= endTime);
        }
        catch (error) {
...
exports.WorkSummaryStorage = WorkSummaryStorage;
//# sourceMappingURL=workSummaryStorage.js.map