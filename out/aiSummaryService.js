...
class AISummaryService {
     ...
     * 生成工作总结
     */
    async generateSummary(commits, historySummaries) {
        if (!this.apiKey) {
     ...
     */
    async callAIWithSystemPrompt(userPrompt, systemPrompt) {
        // 第一次尝试：使用标准格式
        try {
     ...
     * 调用AI接口的通用方法
     */
    async callAI(prompt, mergeMessages) {
        return await this.callAIInternal(prompt, this.getSystemPrompt(), mergeMessages);
    }
    /**
     * 内部AI调用方法
     */
    async callAIInternal(userPrompt, systemPrompt, mergeMessages) {
        ...
        const requestData = {
            model: this.model,
            messages: messages,
            temperature: this.model.includes('reasoner') ? 0.1 : 0.3,
            max_tokens: 2000,
            stream: false
        };
        try {
     ...
     * 构建提示词
     */
    buildPrompt(commits, historySummaries) {
        const config = this.configManager.getConfiguration();
        // 使用自定义用户提示词模板（如果配置了）
...
请按以下 JSON 格式输出：
{
  ...
  "mainTasks": [
     ...
     * 使用自定义模板构建提示词
     */
    buildCustomPrompt(template, commits, historySummaries) {
        // 构建提交信息字符串
        let commitsInfo = '';
        commits.forEach((commit, index) => {
...
- 确保 JSON 格式正确，可以被解析`;
     ...
     * 生成日报或周报
     */
    async generateReport(commits, summaries, reportType, period) {
        if (!this.apiKey) {
            throw new Error('AI API Key 未配置');
        }
     ...
     * 构建报告提示词
     */
    buildReportPrompt(commits, summaries, reportType, period) {
     ...
     * 构建日报提示词
     */
    buildDailyReportPrompt(commits, summaries, dateStr) {
        const config = this.configManager.getConfiguration();
     ...
     * 使用自定义模板构建报告提示词
     */
    buildCustomReportPrompt(template, commits, summaries, options) {
        // 构建提交信息字符串
        let commitsInfo = '';
        if (commits.length > 0) {
            const totalAdditions = commits.reduce((sum, c) => sum + c.additions, 0);
     ...
     * 构建周报提示词
     */
    buildWeeklyReportPrompt(commits, summaries, startDate, endDate) {
        const config = this.configManager.getConfiguration();
     ...
     * 生成多项目报告
     */
    async generateMultiProjectReport(commits, projectStats, reportType, period) {
        try {
     ...
     * 构建多项目报告提示词
     */
    buildMultiProjectReportPrompt(commits, projectStats, reportType, period) {
        const startDate = period.start.toLocaleDateString('zh-CN');
        const endDate = period.end.toLocaleDateString('zh-CN');
     ...
     * 获取报告系统提示词
     */
    getReportSystemPrompt(reportType) {
        const config = this.configManager.getConfiguration();
        // 使用自定义提示词（如果配置了）
...
- 使用中文，格式为标准JSON`;
     ...
     */
    async improveCommitMessage(originalMessage, changes) {
        if (!this.apiKey) {
            throw new Error('AI API Key 未配置');
        }
    ...
    buildCommitMessagePrompt(originalMessage, changes) {
    ...
    buildUncommittedSummaryPrompt(changes) {
