...
class ConfigurationManager {
    getConfiguration() {
        ...
        return {
            ...
            maxFilesPerCommit: config.get('maxFilesPerCommit', 3),
            AIOutputlanguage: config.get('AIOutputlanguage', '中文')
        };
    }
    ...
    async showConfiguration() {
        ...
        // 显示配置面板
        const panel = vscode.window.createWebviewPanel('gitWorkSummaryConfig', 'Git Work Summary Configuration', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async testConfiguration(config) {
        return vscode.window.withProgress({
            ...
            (0, logger_1.log)('📋 Validating basic configuration... | 正在验证基础配置...');
            if (config.interval < 1) {
                ...
                (0, logger_1.log)(`✅ Timer interval: ${config.interval} minutes | 定时间隔: ${config.interval} 分钟`);
            }
            if (config.maxCommits < 1) {
                ...
                (0, logger_1.log)(`✅ Maximum commits: ${config.maxCommits} | 最大提交数量: ${config.maxCommits}`);
            }
            if (config.maxFilesPerCommit < 1 || config.maxFilesPerCommit > 20) {
                ...
                (0, logger_1.log)(`✅ Maximum files per commit: ${config.maxFilesPerCommit} | 每个提交最大文件数: ${config.maxFilesPerCommit}`);
            }
            if (config.aiTimeout < 10 || config.aiTimeout > 300) {
                ...
                (0, logger_1.log)(`✅ AI timeout: ${config.aiTimeout} seconds | AI 超时时间: ${config.aiTimeout} 秒`);
            }
            if (!config.AIOutputlanguage || config.AIOutputlanguage.trim() === '') {
                ...
                (0, logger_1.log)(`✅ AI output language: ${config.AIOutputlanguage} | AI 输出语言: ${config.AIOutputlanguage}`);
            }
            // 2. AI配置测试
            ...
            if (!config.aiApiKey.trim()) {
                ...
                (0, logger_1.log)('✅ AI API Key configured | AI API Key 已配置');
                // 如果AI服务可用，测试连接
                if (this.aiService) {
                    try {
                        // 临时更新AI服务配置用于测试
                        ...
                        (0, logger_1.log)('🔄 Testing AI connection... | 🔄 正在测试AI连接...');
                        const aiTestResult = await this.aiService.testConnection();
                        if (aiTestResult) {
                            (0, logger_1.log)('✅ DeepSeek API access test passed | DeepSeek API 访问测试通过');
                        }
                        else {
            ...
            if (!config.reportUrl.trim()) {
                (0, logger_1.log)('⏭️ Report API URL not configured, skipping report test | 上报接口 URL 未配置，跳过上报测试');
            }
                ...
                (0, logger_1.log)('✅ Report API URL format correct | 上报接口 URL 格式正确');
                // 如果报告服务可用，测试连接
                if (this.reportService) {
                    try {
                        ...
                        (0, logger_1.log)('🔄 Testing report service connection... | 🔄 正在测试上报服务连接...');
                        const reportTestResult = await this.reportService.testConnection();
                        if (reportTestResult.success) {
                            (0, logger_1.log)(`✅ Report service connection successful: ${reportTestResult.message} | 上报服务连接成功: ${reportTestResult.message}`);
                        }
                        else {
            ...
            if (config.enableMultiProject) {
                ...
                if (config.projectPaths.length === 0) {
                    ...
                    (0, logger_1.log)(`✅ Configured ${config.projectPaths.length} project paths | 配置了 ${config.projectPaths.length} 个项目路径`);
                    // 验证项目路径
                    let validProjects = 0;
                    ...
                    (0, logger_1.log)(`✅ ${validProjects}/${config.projectPaths.length} project paths valid | ${validProjects}/${config.projectPaths.length} 个项目路径有效`);
                }
            }
            progress.report({ increment: 10, message: "Test completed | 测试完成" });
            // 显示测试结果汇总
            (0, logger_1.log)('\n📊 Test results summary:');
            (0, logger_1.log)(`├─ 基础配置: ${hasErrors ? '❌ 存在问题' : '✅ 正常'} | ├─ Basic configuration: ${hasErrors ? '❌ Issues' : '✅ Normal'}`);
            (0, logger_1.log)(`├─ AI配置: ${config.aiApiKey ? '✅ 已配置' : '❌ 未配置'} | ├─ AI configuration: ${config.aiApiKey ? '✅ Configured' : '❌ Not configured'}`);
            (0, logger_1.log)(`├─ 上报配置: ${config.reportUrl ? '✅ 已配置' : '⏭️ 未配置'} | ├─ Report configuration: ${config.reportUrl ? '✅ Configured' : '⏭️ Not configured'}`);
            (0, logger_1.log)(`└─ 多项目: ${config.enableMultiProject ? '✅ 已启用' : '⏭️ 未启用'} | └─ Multi-project: ${config.enableMultiProject ? '✅ Enabled' : '⏭️ Disabled'}`);
            // 显示最终结果
            if (hasErrors) {
                (0, logger_1.log)('❌ Configuration test found issues, please check errors and fix configuration | 配置测试发现问题，请检查错误并修复配置');
            }
            else {
                (0, logger_1.log)('🎉 All configuration tests passed! | 所有配置测试通过！');
            }
            // 生成错误报告
