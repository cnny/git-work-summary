...
export class HistoryViewProvider {
    ...
    constructor(private context: vscode.ExtensionContext) {
        this.storage = new WorkSummaryStorage(context);
    ...
    async showHistory(): Promise<void> {
        const panel = vscode.window.createWebviewPanel(
            'gitWorkSummaryHistory',
            'Git Work Summary 历史记录',
        ...
        panel.webview.onDidReceiveMessage(
            async (message) => {
                log(`收到webview消息: ${JSON.stringify(message)}`);
                try {
     ...
     * 加载并显示历史记录
     */
    private async loadAndDisplayHistory(panel: vscode.WebviewPanel): Promise<void> {
        try {
    ...
    private async deleteSummary(id: string): Promise<void> {
        const confirm = await vscode.window.showWarningMessage(
            '确定要删除这个工作总结吗？删除后将重置相关的提交记录，可以重新生成报告。',
            { modal: true },
            '删除'
        );
        ...
        if (confirm === '删除') {
            try {
                ...
                if (deleted && summaryToDelete) {
                    log(`删除工作总结: ${id}, 包含 ${summaryToDelete.commits.length} 个提交`);
                    
                    // 重置Git分析器的最后处理提交记录，以便重新处理这些提交
                    this.gitAnalyzer.resetLastProcessedCommit();
                    log('已重置Git提交处理记录，相关提交可以重新处理');
     ...
     * 导出工作总结
     */
    private async exportSummaries(): Promise<void> {
     ...
     * 显示工作总结详情
     */
    private async showSummaryDetails(id: string): Promise<void> {
        ...
        const panel = vscode.window.createWebviewPanel(
            'gitWorkSummaryDetails',
            `工作总结详情 - ${new Date(summary.timestamp).toLocaleDateString('zh-CN')}`,
     ...
     * 获取历史记录 HTML 内容
     */
    private getHistoryWebviewContent(summaries: WorkSummary[], statistics: any): string {
...
<body>
     ...
     * 获取工作总结详情 HTML 内容
     */
    private getSummaryDetailsWebviewContent(summary: WorkSummary): string {
...
<body>
     ...
     * 获取类型标签（只显示类型）
     */
    private getTypeTag(summary: WorkSummary): string {
     ...
     * 获取日期范围
     */
    private getDateRange(summary: WorkSummary): string {
        const type = summary.type || 'daily'; // 默认为daily
